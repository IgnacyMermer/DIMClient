...
int ret;
	...
	{
#ifdef WIN32
		errno = WSAECONNREFUSED;	/* fake an error code */
	...
	{
#ifndef WIN32
		int ehostnotfoundcode = HOST_NOT_FOUND;
		int enodatacode = NO_DATA;
		ret = h_errno;
#else
		int ehostnotfoundcode = WSAHOST_NOT_FOUND;
		int enodatacode = WSANO_DATA;
		ret = WSAGetLastError();
#endif
		if((ret == ehostnotfoundcode) || (ret == enodatacode))
				return(0);
/*		
		errno = ECONNREFUSED;
#ifdef WIN32
...
#endif
#ifndef WIN32
	int etimedoutcode = ETIMEDOUT;
#else
	int etimedoutcode = WSAETIMEDOUT;
#endif
	int so_error = etimedoutcode;
...
*/
				closesock(path);
#ifdef WIN32
				errno = WSAEADDRNOTAVAIL;	/* fake an error code */
				WSASetLastError(errno);
#else
...
{
#ifndef WIN32
	int ewouldblockcode = EWOULDBLOCK;
	int einprogresscode = EINPROGRESS;
#else
	int ewouldblockcode = WSAEWOULDBLOCK;
	int einprogresscode = WSAEINPROGRESS;
#endif
	if(code == ewouldblockcode)
		return(1);
   else if (code == einprogresscode)
	   return(1);
   return(0);
}
