...
char dipServiceName[MAX_CHAR_LEN];


int main(int argc, char** argv)
{
//	const char* to_node = "localhost";
//	const int port = 2505;
//	int nServices = atoi(argv[1]);
	int nServices = 10000;

//	DimServer::setDnsNode(to_node, port);
	DimServer::start("testServer");

	cout << "Creating " << nServices << " DIP publications." << endl;

	for (int i = 0; i < nServices; i++) {
		sprintf(dipServiceName, "testService_%i/DipData", i);
		new DimService(dipServiceName, "C", "A", 2);
		sprintf(dipServiceName, "testService_%i/DipType", i);
		new DimService(dipServiceName, "C", "A", 2);
	}
	while (1)
		pause();
...
*/
/*
class StateCmd : public DimCommand
{
	DimService stateSrvc;
	char state[64];
	void commandHandler()
	{
		char *cmd;
		...
		if(!strcmp(cmd,"Configure"))
			strcpy(state,"READY");
		cout << "Command " << cmd << " received -> New State "<< state <<endl;
		stateSrvc.updateService((char *)state);
	}

public:
	StateCmd() : DimCommand("eb0N/command", "C"),
		stateSrvc("eb0N/state", (char *)"NOT_READY") {};
};

int main()
{
	StateCmd services;
	DimServer::start("eb0N");
	while (1)
		pause();
}
*/
