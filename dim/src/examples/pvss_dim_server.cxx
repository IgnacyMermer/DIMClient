...
class RecvCommandComplexStr : public DimCommand
{
	void commandHandler()
	{
		COMPLEXSTR *complexData;

		complexData = (COMPLEXSTR *)getData();
		cout << "Command " << complexData->oper
			<< " received " << complexData->stringval << " size "<< getSize() << endl;
	}
public:
	RecvCommandComplexStr(const char *name) : DimCommand(name, "C:1;C") {};
};
/*
typedef struct{
	char oper;
...
int main()
{
	...
	RecvCommandComplexStr recvCommandComplexStr("COMPLEX_COMMAND_STR");
	/*
	{
	char tstr[128];
	int i;
