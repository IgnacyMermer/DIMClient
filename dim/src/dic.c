...
static DIC_DNS_CONN *DNS_head = (DIC_DNS_CONN *)0;
static DIC_DNS_CONN *Default_DNS = 0;
...
static DIC_DNS_CONN *create_dns(dim_long dnsid)
{
	DIC_DNS_CONN *dnsp;

	dnsp = malloc(sizeof(DIC_DNS_CONN));
...
DIC_DNS_CONN *dic_find_dns(dim_long dnsid)
{
	DIC_DNS_CONN *dnsp;
...
*/
			...
			else if(dnsp->dns_dic_conn_id <= 0)
			{
				service_tmout( servp->serv_id );
			}
/*
...
static void recv_dns_dic_rout( int conn_id, DNS_DIC_PACKET *packet, int size, int status )
{
	...
	case STA_DISC:       /* connection broken */
		...
		dna_close(dnsp->dns_dic_conn_id );
		dnsp->dns_dic_conn_id = 0;
		request_dns_info(0);
		break;
...
/*
dim_print_date_time();
printf("In service tmout %s, %d, %d\n", servp->serv_name, servp->timeout, servp->curr_timeout);
*/
	servp->tmout_done = 1;
...
void dic_dns_init()
{
	...
	if(!done)
	{
		...
		if(!DNS_head) 
		{
			DNS_head = (DIC_DNS_CONN *)malloc(sizeof(DIC_DNS_CONN));
...
void dic_release_service( unsigned service_id )
{
	...
	case WAITING_SERVER_UP :
		...
		if(dnsp->dns_dic_conn_id > 0) {
			...
			dna_write(dnsp->dns_dic_conn_id, dic_dns_p,
...
//		ptr = servp;
		if(id > 0)
		{
			...
			{
				if ((ptr->serv_id == id) && 
				    (ptr->pending != NOT_PENDING) &&
				    (ptr->pending != WAITING_SERVER_UP))
					servp = ptr;
				else
					ptr = 0;
			}
						...
						(DLL *) servp)) )
		{
		  /*
		  dim_print_date_time();
		  printf("request dns info? %s %d \n         head %lx headN %lx headP %lx servp %lx next %lx prev %lx\n", servp->serv_name, n_pend, Service_pend_head, Service_pend_head->next, Service_pend_head->prev, servp, servp->next, servp->prev);
		  */
			...
			if(n_pend == 1000)
			{
				...
				/*
				dim_print_date_time();
				printf("exiting request dns info 1000\n");
				*/
				ENABLE_AST
				return;
			}
...
*/
		/*
	dim_print_date_time();
	printf("exiting request dns info\n");
		*/
	ENABLE_AST
}
...
void move_to_ok_service( DIC_SERVICE *servp, int conn_id )
{
	if(Dic_conns[conn_id].service_head)
	{
		DISABLE_AST
		  /*
dim_print_date_time();
printf("move_to_ok %s\n         head %lx headN %lx headP %lx servp %lx next %lx prev %lx\n", servp->serv_name, Service_pend_head, Service_pend_head->next, Service_pend_head->prev, servp, servp->next, servp->prev);
		  */
//printf("move_to_ok %s\n",servp->serv_name);
...
/*
dim_print_date_time();
	printf("move_to_bad %lx %s\n         head %lx headN %lx headP %lx servp %lx next %lx prev %lx\n", bad_connp->conn.service_head, servp->serv_name, Service_pend_head, Service_pend_head->next, Service_pend_head->prev, servp, servp->next, servp->prev);
