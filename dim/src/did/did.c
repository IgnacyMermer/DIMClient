...
int main(int argc, char *argv[])
{
	...
	if(argc > 1)
	{
		...
		if(argv[i][0] == '-')
		{
			...
			if((!strncmp(opt_str,"node",4)) || 
			...
			else if((!strncmp(opt_str,"port",4)) || 
				(!strncmp(opt_str,"PORT",4))) {
			        char text[132], *end = NULL;
			        sprintf(text,"DIM_DNS_PORT=%s",opt_str+5);
   		  	        putenv(text);
 			        dim_set_dns_port(strtol(opt_str+5, &end, 10));
			}
				...
				(!strncmp(opt_str,"HELP",4)))
			  {
                                 ...
                                 printf("\t-port=<number>   Show Servers with DIM_DNS_PORT provided by <number>\n");
