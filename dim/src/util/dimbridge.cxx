...
//	cout << "Sending Command " << srv_name << endl;
...
void print_usage()
{
	cout << "DimBridge version " << DIM_VERSION_NUMBER << endl;
...
//		cout << "arg " << i << " " << argv[i] << endl;
	...
	get_node_name(node);
	dim_print_date_time();
	cout << "DimBridge version " << DIM_VERSION_NUMBER << " starting up on " << node << ", bridging:" << endl;
	cout << "from " << from_node << " to " << to_node << endl;
...
// Register Commands first
		while ((servp = bridge_hash_service_get_next(&hash_index, servp, 0)))
		{
			...
			else if ((type == DimSERVICE) && (known == 2))
			{
				ptrs = (BridgeService *)servp->browserServicePtr;
				if (ptrs)
				{
					if (ptrs->isDeclared() == 1)
						done = 1;
				}
			}
		}
		...
		{
			dim_print_date_time();
			cout << "Start Serving " << bridge_name << endl;
			DimServer::start(bridge_name);
		}
