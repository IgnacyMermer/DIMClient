...
int did_init(char *local_node, int dns_port)
{
	...
	{
		snprintf(Title, sizeof(Title), "DIM DNS: %s:%d",dns_node,dns_port);
	}
	else
	{
		snprintf(Title, sizeof(Title), "DIM DNS: %s",dns_node);
	}
	snprintf(icon_title,sizeof(icon_title),"DID %s",dns_node);
	if (IsWebDID)
	{
		dim_print_date_time();
...
*/
				...
				if (*stype == '\n')
					...
					type = 2;
				snprintf(str, sizeof(str), "name: \"%s\", format: \"%s\", type: %d", sptr, sformat, type);
...
NODE *nodep;
int j, test;
char str[256+MAX_NAME], sname[MAX_NAME], *ptr;
int prepareJSONTree();
...
void got_update_services();
	...
	if(n_services != 0)
	{
		...
		if (n_services != -1)
		{
			...
			snprintf(str, sizeof(str), "%s/SERVICE_LIST", sname);
...
char *update_services(char *node, char *server, int pid, int browser)
{
	...
	if(server){}
	snprintf(str, sizeof(str),"%s/SERVICE_LIST",server);
	dic_info_service(str,ONCE_ONLY,20,0,0,
...
printf("CMD data %s %s\n",service,format);
	...
	snprintf(answer,sizeof(answer),"To %s (%s)",service, format);
...
time_t tsecs;
	...
	if(size < 1024)
		...
		snprintf(sizestr,sizeof(sizestr),"%2.2f Kb",(float)size/1024);
	else
		snprintf(sizestr,sizeof(sizestr),"%2.2f Mb",(float)size/(1024*1024));
    ...
    ptr = strrchr(timestr,' ');
    strcpy(aux, ptr);
    snprintf(ptr,5,".%03d",mili);
    strcat(timestr, aux);
    timestr[(int)strlen(timestr)-1] = '\0';
   
    snprintf(str, sizeof(str)," Timestamp: %s&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Quality: %d&nbsp&nbsp&nbsp Size: %s<br /><br />",
...
void sprintf_html(char *str, int n, int value)
{
	...
	str[0] = '\0';
	min = snprintf(tmp,sizeof(tmp),"%d",value);
	for(i = 0; i < (n-min); i++)
	{
		strcat(str,"&nbsp");
	}
...
int last[4];
	...
	for( i = 0; i < size; i++)
	{
	  strcpy(tmp,"");
		...
		if(!(i%4))
			strcat(tmp,"H: ");
		snprintf(str, sizeof(str),"&nbsp&nbsp&nbsp %08X",buff[i]);
		strcat(tmp,str);
		last[i%4] = buff[i];
		if((i%4 == 3) || (i == (size-1)))
		{
...
longlong last[4];
	...
	for( i = 0; i < size; i++)
	{
	  strcpy(tmp,"");
		...
		if(!(i%4))
			strcat(tmp,"H: ");
		snprintf(str, sizeof(str),"&nbsp&nbsp&nbsp %08X",(unsigned)buff[i]);
		strcat(tmp,str);
		last[i%4] = buff[i];
		if((i%4 == 3) || (i == (size-1)))
		{
...
short last[8];
	...
	for( i = 0; i < size; i++)
	{
	  strcpy(tmp,"");
		...
		if(!(i%8))
			strcat(tmp,"H: ");
		snprintf(str, sizeof(str),"&nbsp %04X",buff[i]);
		strcat(tmp,str);
		last[i%8] = buff[i];
		if((i%8 == 7) || (i == (size-1)))
		{
...
char *asc, *ptr, str[80], tmp[256];
	...
	for( i = 0; i < size; i++)
	{
	  strcpy(tmp,"");
		...
		if(!(i%16))
			strcat(tmp,"H: ");
		snprintf(str, sizeof(str),"%02X",buff[i]);
/*		strcat(tmp,str);
*/
		strcat(tmp," ");
		...
		*/
		if((i%16 == 15) || (i == (size-1)))
		{
			...
			for(j = 0; j <= (i%16) ; j++)
			{
				...
				{
					if(asc[j] == ' ')
						snprintf(str, sizeof(str),"&nbsp");
					else if(asc[j] == '<')
						snprintf(str, sizeof(str),"&lt");
					else if(asc[j] == '>')
						snprintf(str, sizeof(str),"&gt");
					else if(asc[j] == '&')
						snprintf(str, sizeof(str),"&amp");
					else
						snprintf(str, sizeof(str),"%c",asc[j]);
					strcat(tmp,str);
				}
				else
				{
...
char *ptr, str[80], tmp[256];
	...
	for( i = 0; i < size; i++)
	{
	  strcpy(tmp,"");
		...
		snprintf(str, sizeof(str),"%12.3G",*(buff++));
		strcat(tmp,str);
		if((i%4 == 3) || (i == size-1))
		{
			strcat(tmp,"<br />");
...
int prepareJSONTree(char *node, BROWSER *browserp)
{
	...
	NODE *nodep;
	SERVER *servp;
	char str[512], aux[128];
	int selective = 0;
	int n_nodes, tot_n_nodes = 0;
	int n_servers, tot_n_servers = 0;
	int ret, n_found = 0;
	...
	*ptr = '\0';
	...
	ptr = addJSONNodeStart(ptr,"children");
		snprintf(str, sizeof(str),"text: \"%s\", id: \"Nodes\", expanded: false", Title);
	...
	while( (nodep = (NODE *)sll_get_next((SLL *)nodep)) )
	{
		...
		snprintf(str, sizeof(str),"text: \"%s\", id: \"%s\", qtip: \"%s\"",
	...
	ptr = addJSONNodeStart(ptr,"children");
		...
		while( (servp = (SERVER *)sll_get_next((SLL *)servp)) )
		{
			if(!servp->match)
				continue;
			snprintf(str, sizeof(str),"text: \"%s\", id: \"%d\", leaf: true, icon: \"server.png\"",servp->name, servp->server.pid);
			ptr = addJSONChildStart(ptr,str,0);
			n_servers++;
	...
	*ptr = '\0';
	...
	ptr = addJSONNodeStart(ptr,"children");
		...
		while( (servp = (SERVER *)sll_get_next((SLL *)servp)) )
		{
			...
			if(sptr)
				*sptr = '\0';
			snprintf(str, sizeof(str),"text: \"%s\", id: \"%d\", leaf: true, icon: \"server.png\", name: \"%s\"",aux, servp->server.pid, servp->name);
			ptr = addJSONChildStart(ptr,str,0);
			n_servers++;
			if(n_servers < tot_n_servers)
				ptr = addJSONChildEnd(ptr,1);
...
int prepareJSONDimTree(char *node, BROWSER *browserp)
{
	...
	int selective = 0;
	int tot_n_nodes = 0;
	int n_servers = 0, tot_n_servers = 0;
	int ret, n_found = 0;
...
int prepareJSONHeader()
{
	...
	*ptr = '\0';
	...
	snprintf(str, sizeof(str),"text: \"%s\"",Title);
	ptr = addJSONChildStart(ptr,str,0);
	ptr = addJSONChildEnd(ptr,1);
	snprintf(str, sizeof(str),"text: \"%d Servers Known - %d Services Available\"",N_servers, N_services);
	ptr = addJSONChildStart(ptr,str,0);
	ptr = addJSONChildEnd(ptr,0);
	ptr = addJSONNodeEnd(ptr);
	ptr = addJSONEnd(ptr);
...
char *prepareJSONServiceList(SERVER *servp, char *node, int pid, BROWSER *browserp)
{
	...
	*ptr = '\0';
	...
	ptr = addJSONNodeStart(ptr,"children");
	if(selective)
		snprintf(str, sizeof(str),"text: \"%s (%d/%d services, pid %d)\"",servp->name, n_found, n_services, servp->server.pid);
	else
		snprintf(str, sizeof(str),"text: \"%s (%d services, pid %d)\"",servp->name, n_services, servp->server.pid);
    ptr = addJSONChildStart(ptr,str,1);
	ptr = addJSONNodeStart(ptr,"children");
	servicep = servp->service_ptr;
	n = 0;
...
*/
		if((!selective) || (strstr(servicep->name, browserp->pattern)))
		{
			if(servicep->type == 1)
			{
				...
				snprintf(str, sizeof(str),"text: \"%s\", id: \"%s\", leaf: true, icon: \"leaf_cmd.gif\"",servicep->name, type_str);
			}
			else
			{
				sprintf(type_str,"%d@%s|%s", pid, node, servicep->name);
				snprintf(str, sizeof(str),"text: \"%s\", id: \"%s\", leaf: true",servicep->name, type_str);
			}
			ptr = addJSONChildStart(ptr,str,0);
			n++;
			if(n < n_found)
...
printf("prepareJSONSmiObjectList1 name %s\n", servp->name);
	...
	*ptr = '\0';
	...
	/*
	if(selective)
		snprintf(str, sizeof(str),"name: \"%s (%d/%d services, pid %d)\"",servp->name, n_found, n_services, servp->server.pid);
	else
		snprintf(str, sizeof(str),"name: \"%s (%d services, pid %d)\"",servp->name, n_services, servp->server.pid);
	*/
	snprintf(str, sizeof(str),"name: \"%s (%d objects, pid %d)\"",servp->name, n_found, servp->server.pid);
    ptr = addJSONChildStart(ptr,str,1);
	ptr = addJSONNodeStart(ptr,"children");
	servicep = servp->service_ptr;
	n = 0;
...
/*
			...
			{
				sprintf(type_str,"%d@%s|%s|CMD", pid, node, servicep->name);
				snprintf(str, sizeof(str),"name: \"%s\", id: \"%s\", leaf: true, icon: \"leaf_cmd.gif\"",servicep->name, type_str);
			}
			else
			{
				sprintf(type_str,"%d@%s|%s", pid, node, servicep->name);
				snprintf(str, sizeof(str),"name: \"%s\", state: \"RUNNING\", id: \"%s\", leaf: true",servicep->name, type_str);
			}
*/
...
//				snprintf(str, sizeof(str),"name: \"%s\", state: \"%s\", id: \"%s\", leaf: true, fname: \"%s\"",sptr, state, type_str, servicep->name);
				snprintf(str, sizeof(str),"name: \"%s\", state: \"%s\", mode: \"%s\",id: \"%s\", leaf: true, fname: \"%s\"",
