                    ...
                    DIM version 20r37 Release Notes
...
NOTE 3: The Version Number service provided by servers is now set to 2037.

06/02/2024
Changes for version 2037:
    - Fixed another race condition when an application had a large number of connections.


02/02/2024
Changes for version 2036:
    - Fixed a race condition in dtq_stop_timer that could lead to crashes in multithreaded applications.
    - The Linux DID now accepts an extra parameter: -port=<port_number> (to add to -dns=<dns_node_name>)
    - Fixed several new warnings on gcc 11.3/RHEL9
    - Did is no longer statically linked, and the X/Motif libraries changed place (RHEL9 issues)


20/01/2023
Changes for version 2035:
    - Replaced Mutexes by Critical_sections in Windows to improve speed, as per CMS suggestion.
    - One of the changes in the last version could provoke a crash of the client, fixed.


09/11/2022
Changes for version 2034:
    - Fixed yet another bug related to servers publishing to different DNSs.
    - Tried to fix a crash in CMSs Bridge, related to "once only" getServerServices.


07/04/2022
Changes for version 2033:
    - Fixed the DimBridge, to bridge commands before services 
      (to guarantee that if a user sends a command inside a service callback, the command is also available) 
    - and to declare commands and services to the "to" DNS in batches instead of one by one.


21/03/2022
Changes for version 2032:
    - Cosmetic changes to DimBridge: adding version number


21/03/2022
Changes for version 2031:
    - Fixed a bug for servers and clients connecting to the Dns when using multiple DNS functions.
    - Clients could (very rarely) hang when power cycling a server machine - fixed.
...
Changes for version 2027:
    - Harmonized the addDns() functions between servers/clients.
    - Allow "start_serving" with a null dnsid.
    - Fixed some compiler warnings, thanks to Markus.
    - Optimised the DIM client when requesting DNS information about many services.


03/06/2019
Changes for version 2026:
    - Fixed a bug introduced in the latest version:
      Dim clients would crash when setting DIM_DNS_NODE at startup.


08/05/2019
Changes for version 2025:
    - A new improved version of DIMTree for Windows is back! 
      It's a 32 bit version (in bin32 folder) but works fine on both (32 and 64 bit platforms)
      Thanks to Alexei Kuryakin
    - Fixed Windows version of dim_usleep(), could fail at startup.
    - Fixed WebDID for Windows10.
    - DIM Clients can now use multiple DIM_DNSs, in the same way Servers can:
	- By calling (for example) in C:
		dim_long dnsid = dic_add_dns("newdnsname", newportnumber);
		dic_info_service_dns(dnsid, ...)
		dic_cmnd_service_dns(dnsid, ...)
	- By calling (for example) in C++:
		dim_long dnsid = DimClient::addDns("lxplus613.cern.ch", 0);
		DimInfo servint(dnsid, "TEST/INTVAL", -1);
		DimBrowser br = new DimBrowser(dnsid);
		int n = br->getServices("*");


13/12/2018
Changes for version 2024:
    - dna.c: better handling and reporting of pending connections.
    - dtq.c: changed dim_usleep to use select() for both windows and linux.
             This select call can now be interrupted when an "immediate" timer request is received. 
             (The problem, reported by ALICE, was that RPC calls were blocked for 100 ms) 
